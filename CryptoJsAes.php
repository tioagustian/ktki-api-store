...
class CryptoJsAes
{
    ...
    public static function encrypt($value, string $passphrase)
    {
        ...
        $data = ["chipertext" => base64_encode($encrypted_data), "iv" => bin2hex($iv), "salt" => bin2hex($salt)];
    ...
    public static function decrypt(string $jsonStr, string $passphrase)
    {
        ...
        $iv = hex2bin($json["iv"]);
        $chipertext = base64_decode($json["chipertext"]);
        ...
        $data = openssl_decrypt($chipertext, 'aes-256-cbc', $key, true, $iv);
